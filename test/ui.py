...
def test_activate(selenium, device_host,
    ...
    wait_for(selenium, lambda: selenium.find_by_id('btn_syncloud_domain').is_displayed())
    selenium.screenshot('activate-empty')
    selenium.find_by_id('btn_syncloud_domain').click()
    ...
    defocus(selenium)
    selenium.screenshot('activate-account')
    selenium.find_by_id('btn_account_next').click()
    wait_for(selenium, lambda: selenium.find_by_id('domain_input').is_displayed())
    selenium.find_by_id('domain_input').send_keys(domain)
    defocus(selenium)
    selenium.screenshot('activate-type')
    selenium.find_by_id('btn_domain_next').click()
    wait_for_loading(selenium.driver)
