...
class ManualPortMapper:
    ...
    def add_mapping(self, local_port, external_port, protocol):
        self.logger.warn('adding port mapping is not available in manual mode {0}, {1}, {2}'.format(
            self.manual_public_ip, self.manual_certificate_port, self.manual_access_port))
        if local_port == 80:
            return self.manual_certificate_port
        elif local_port == 443:
