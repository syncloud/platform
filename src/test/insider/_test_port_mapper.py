...
def test_add_success():
    ...
    when(cmd).run('hostname -I').thenReturn('192.168.1.2 \n')

    mapper = UpnpPortMapper(cmd)
    external_port = mapper.add_mapping(80)

    assert 80 == external_port
...
def test_first_gap():
    ...
    ports_to_try = mapper.__find_available_ports(existing_ports, 2000, 3)
...
def test_no_gap():
    ...
    assert [2003, 2004, 2005] == mapper.__find_available_ports(existing_ports, 2000, 3)
...
def test_no_existing():
    ...
    mapper = PortMapper(port_config, upnpc)
    existing_ports = []
    assert [2000, 2001, 2002] == mapper.__find_available_ports(existing_ports, 2000, 3)
...
def test_open_port():
    ...
    mapper = PortMapper(port_config, upnpc)
    existing_ports = []
    assert [2001, 2002, 2003] == mapper.__find_available_ports(existing_ports, 1999, 3)
...
def test_local_port():
    ...
    mapper = PortMapper(port_config, upnpc)
    existing_ports = [2000, 2001, 2003]
    assert [80, 2002, 2004] == mapper.__find_available_ports(existing_ports, 80, 3)
