...
if [ "$#" -ne 6 ]; then
    echo "usage $0 redirect_user redirect_password redirect_domain app_version app_arch sam_version"
    exit 1
fi
...
py.test -s verify.py --email=$1 --password=$2 --domain=$3 --app-version=$4 --arch=$5
