...
test('activate', async ({}, testInfo) => {
  ...
  await page.locator('#device_username').pressSequentially(deviceUser)
  await page.locator('#device_password').pressSequentially(devicePassword)
  await page.locator('#device_password_confirm').pressSequentially(devicePassword)
  await page.locator('#btn_activate').scrollIntoViewIfNeeded()
